...
describe("claude process identification", () => {
  ...
  test("rejects a bundled tool that re-execs the claude binary", () => {
    ...
    // nor is it one of the helpers — the rule sits in the executable half both
    // predicates share, and the orphan-port scan reads the helper one
    expect(isClaudeHelper(proc("ugrep", "/u/claude/versions/2.1.209"))).toBe(
      false,
    );
